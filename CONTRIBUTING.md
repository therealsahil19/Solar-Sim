...
- **Strict Arrays**: `"noUncheckedIndexedAccess": true` is enabled. You must handle `undefined` potential when accessing array elements by index (e.g. using `??` or checking for existence).
...
- **Performance ("Bolt")**:
  ...
  - Use direct property access on `THREE.BufferAttribute` (e.g. `.getX(i)`) instead of allocating intermediate `THREE.Vector3` objects.
  - Read directly from matrix elements (e.g. `elements[12]`) when possible instead of using `.setFromMatrixPosition()`.
  - Use Object Pooling (e.g. single temporary vectors like `_tempVec2`) to minimize garbage collection (GC) pressure.
...
- **Python Tests (`tests/`)**: Python scripts, like the texture downloader, have their tests here. Note that files in the root `tests/` directory are currently ignored by Playwright and Vitest. They must be run using `python3 -m unittest discover tests`.
