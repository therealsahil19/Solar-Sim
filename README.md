...
6. **Core Code Execution ("Micro-optimizations")**:
    ...
    - **Caching Orbital Positions**: Using `WeakMap` to cache computed positions per frame, eliminating redundant multi-zone math for hierarchical bodies.
    - **Direct Matrix Access**: Bypassing `setFromMatrixPosition` overhead by extracting positions directly via `elements[12]`, `elements[13]`, `elements[14]` (yielding ~1.4x-1.8x speedup).
    - **Primitive Getters**: Bypassing intermediate `THREE.Vector3` object allocations when parsing `THREE.BufferAttribute` data by reading components with `.getX(i)`, `.getY(i)`, and `.getZ(i)`.
    - **Object Pooling**: Utilizing single-instance vectors (like `_tempVec2` in `TrailManager`) to dramatically decrease garbage collection pressure in high-frequency rendering loops.
    - **DOM Element Caching**: Local caching of hot-path DOM elements (e.g., `infoDistSunEl`) prevents repeated `document.getElementById` calls during simulation updates.
    - **for...in loops**: Optimized object cleanup using `for...in` with `hasOwnProperty` checks to bypass array allocations generated by `Object.values().forEach()`, saving GC pauses.
...
- **Security Headers**: The app enforces `X-Content-Type-Options`, `X-Frame-Options`, `X-XSS-Protection`, `Permissions-Policy`, and `Strict-Transport-Security` to further mitigate injection and clickjacking attacks.
- **Strict Configuration Loading**: `system.json` is validated to ensure it originates from a same-origin domain (or strictly `http/https`), ends with `.json`, and explicitly blocks path traversal sequences (`..`) or sensitive root file lookups (`package.json`).
...
11. **`src/components/CommandPalette.ts`**:
...
12. **`src/components/InfoPanel.ts`**:
...
14. **`src/components/NavigationSidebar.ts`**:
...
17. **`src/managers/ToastManager.ts`**:
...
- **pnpm** (strict requirement, replacing npm)
