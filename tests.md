...
5. **Python Script tests:** The Python script `download_textures.py` has a test suite located at `tests/test_download_textures.py`. This suite verifies connection timeouts and executes successfully via `python3 -m unittest discover tests`.
6. **Misplaced Test Files:** Files directly in the root `tests/` directory (e.g., `tests/InstanceRegistry.spec.ts` and `tests/Modal.spec.ts`) are currently ignored by both the Vitest (`tests/unit/`) and Playwright (`tests/e2e/`) test execution configurations. These files should be moved to the appropriate subdirectories.
